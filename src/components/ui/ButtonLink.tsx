...
function ButtonLink({
  ...
  return (
    <Link 
    href={data.page?.slug ? data.page.slug : data.url ? data.url : '/'} target={data.isExternal? '_blank' : ''} className={`py-4 group/button cursor-pointer theme-transition-300 flex w-fit items-center justify-center rounded-[32px] border border-transparent bg-${data.theme} px-6 text-center text-h6 text-${data.theme}-foreground hover:rounded-[32px] hover:border hover:border-${data.theme} hover:bg-transparent hover:text-neutral-700 sm:px-8`}>
