...
function CustomLink({
    text, url, isExternal, textColor, page
}: Readonly<LinkProps>) {
  return (
    <Link href={page?.slug ? page.slug : url? url : '/'} target={isExternal? '_blank' : ''} className={`group/link cursor-pointer theme-transition-500 flex w-fit items-center gap-1 border-b border-neutral-700 pb-1 font-semibold hover:border-secondary text-${textColor? textColor : 'black'}`}>
